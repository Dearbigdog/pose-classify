...
def neural_network_classify(x,para):
	...
	f_combine = np.concatenate((np.reshape(f_angle_3d, (30,)), np.reshape(f_angle_2d,(13,))), axis=0)
...
def restore_parameters():
	...
	num_input = feature_dim=43  # data input
	...
	para=[]
	para.append(sess.run(h1))
	para.append(sess.run(h2))
	para.append(sess.run(h3))
	para.append(sess.run(b1))
	para.append(sess.run(b2))
	para.append(sess.run(b3))
	return para
