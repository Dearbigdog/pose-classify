...
import neural_network.analysis_helper as ah
...
class BodyGameRuntime(object): 
    def __init__(self):
        ...
        pygame.display.set_caption("kinect joint map")
    ...
    def draw_body_bone(self, joints, jointPoints, color, joint0, joint1):
        ...
        try:
            pygame.draw.line(self._frame_surface, color, start, end, 9)
            ...
            if self._bodies is not None: 
                for i in range(0, self._kinect.max_body_count):
                    ...
                    if not body.is_tracked: 
                        continue 
                    joints = body.joints 
					#change the joints to array (25,3)
                    list_x=[]
                    for j in range(0,24):
                    ...
                    plt.axis()
                    plt.ion()
                    plt.plot(range(0, len(avg_feature_akimbo)), avg_feature_akimbo, 'ro--',alpha=0.3)
                    plt.plot(range(0, len(avg_feature_hug)), avg_feature_hug, 'go--',alpha=0.3)
                    plt.plot(range(0, len(f_combine)), f_combine, 'bo--',alpha=1)
                    plt.xticks(np.arange(0, len(f_combine), 1.0))
                    title_obj=plt.title(res)
                    plt.getp(title_obj)
                    if (is_correct):
