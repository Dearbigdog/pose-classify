...
logits=np.load('.\\test_logits.txt')
test_x_path=np.load('.\\test_x_path.txt')
feature_pos=np.load('.\\feature_pos.txt')
...
index_tracer=[]
for i in range(len(test_correct_pred)):
...
for i in index_tracer:
	...
	for j in range(len(test_x_data[i])):
	...
	plt.plot(range(0, len(test_x[i])), test_x[i], 'ro--')
    ...
    #show average feature line
    #show standard pose combined with hug and cross
	fig.add_subplot(2,2,4)
	plt.title('average feature line')
	axis_x=range(1,43)

	sum_feature=np.zeros(43)
	avg_feature=np.zeros(43)
	for f in range(0,len(feature_pos)):
		for p in range(0,len(feature_pos[f])):
			sum_feature[p]+=feature_pos[f][p]
	for m in range(0,43):
		avg_feature[m]=sum_feature[m]/len(feature_pos)
	plt.plot(range(0, len(avg_feature)), avg_feature, 'bo--')
...
for i in range(0, len(test_correct_pred), 1):
	fig = plt.figure(figsize=plt.figaspect(1))
